...
import { getMdLeaveRequests, mdActionLeaveRequest } from '../services/api.js';
...
export default function MdDashboard() {
  ...
  const load = useCallback(async () => {
    try {
      const all = await getMdLeaveRequests();
      if (Array.isArray(all)) {
        setRequests(all);
        setLoadError('');
      } else {
        // getMdLeaveRequests() swallows errors and returns [] on failure —
        // this branch only fires if the shape ever changes, but keep it
        // defensive so a bad response doesn't just look like "no requests".
        setRequests([]);
        setLoadError('Unexpected response while loading leave requests.');
      }
    } catch (error) {
      console.error('Failed to load MD leave requests:', error);
      setLoadError('Could not load leave requests — check your connection and try again.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handlePrint = () => {
    ...
    <html>
      ...
      <body>
        ...
        <h3 style="text-align:center;">Managing Director — Leave History Report</h3>
        ...
        <div class="footer">
          Total On Leave: ${dateRequests.length}
        </div>
      </body>
    </html>
  `);
  ...
  const handleAction = async (requestId, action) => {
    ...
    const res = await mdActionLeaveRequest(requestId, action, remarksDraft[requestId] || '');
    setBusyId(null);
    if (res.ok) {
      setToast({
        type: 'ok',
  ...
  const renderActions = (r) => {
    if (r.status === 'PENDING') {
      return (
        <>
          <input
            className="input" placeholder="Optional remark…"
      ...
      <main className="main-area">
        <div className="container">
          <div className="page-header anim-fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
            <div>
              <h1>Managing Director — Approval Dashboard</h1>
              <p>Review leave requests submitted by Heads of Department, and your own.</p>
            </div>
          ...
          {view === 'MINE' ? (
            <MyLeaveRequests
              empId={session?.user?.mdId || session?.user?.empId}
              title="My Leave Requests"
              subtitle="There's no one above the Managing Director, so you approve your own leave requests yourself from the 'Requests to Approve' tab."
              showBalances
            />
          ) : (
          <>
                    ...
                    <div className="emp-cell">
                      ...
                      <div>
                        <div className="ec-name">
                          {r.empName} <span style={{ color: 'var(--ink-300)', fontWeight: 500 }}>· {r.empId}</span>
                          {String(r.empId) === String(session?.user?.mdId) && (
                            <span className="badge" style={{ marginLeft: 6 }}>You</span>
                          )}
                        </div>
                        <div className="ec-sub">{r.department || 'Head of Department'}</div>
                      </div>
                    </div>
